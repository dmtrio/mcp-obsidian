...
async function watchReady(watcher, folder, cursorId, agentName) {
...
describe('FileWatcherService', () => {
    ...
    describe('cursor management', () => {
        ...
        it('updates cursor seen comments', () => {
            ...
            const seenComments = new Map([
                ['c_abc123', { replyCount: 2, status: 'open', lastActivityAt: new Date().toISOString() }]
            ]);
    ...
    describe('watch', () => {
        it('detects sidecar file creation', async () => {
            ...
            await writeFile(join(vaultPath, 'project', 'note.md.comments.json'), JSON.stringify({ version: 1, comments: [] }));
        ...
        it('returns cursor for subsequent calls', async () => {
            ...
            await writeFile(join(vaultPath, 'project', 'note.md.comments.json'), JSON.stringify({ version: 1, comments: [{ id: 'c_new' }] }));
    ...
    describe('debouncing', () => {
        it('collects rapid changes into single result', async () => {
            ...
            await writeFile(join(vaultPath, 'project', 'note1.md.comments.json'), JSON.stringify({ version: 1, comments: [] }));
            await writeFile(join(vaultPath, 'project', 'note2.md.comments.json'), JSON.stringify({ version: 1, comments: [] }));
            const result = await watchPromise;
            expect(result.timedOut).toBe(false);
